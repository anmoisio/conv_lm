  ...
  foreach my $f (@syncfiles) {
    ...
    while (! -f $f) {
      ...
      if (($check_sge_job_ctr++ % 10) == 0) { # Don't run qstat too often, avoid stress on SGE.
        ...
        if ($squeue_status == 1) {
          # Wait a couple of seconds. It might be just that we didn't get the status.
          sleep(4);
          ($squeue_output, $squeue_status) = exec_command("squeue -j $sge_job_id");
        }
        if ($squeue_status == 1) {
          # The job does not seem to exist.
          ...
          my $queue_last_line = `tail -n 1 $queue_logfile`;
          if ($last_line =~ m/status 0$/ && (-M $logfile) < 0) {
            ...
            print STDERR "$0: Error: Job $sge_job_id seems to no longer exists:\n" .
              ...
              "Queue log is in $queue_logfile. Last line is '$queue_last_line'.\n" .
              "Possible reasons:\n" .
              "  a) Exceeded time limit? -> Use more jobs!\n" .
              "  a) Exceeded memory limit? -> Increase memory limit!\n" .
