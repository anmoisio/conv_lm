...
convert_kn () {
	...
	local model_file="${EXPT_WORK_DIR}/${model_name}.arpa"
	[ -s "${model_file}" ] || local model_file="${EXPT_WORK_DIR}/${model_name}.arpa.gz"
	[ -s "${model_file}" ] || { echo "Model file not found." >&2; exit 1; }

	local model_bin_file="${EXPT_WORK_DIR}/${model_name}.fsabin"
	local lookahead_file="${EXPT_WORK_DIR}/${model_name}-lookahead.arpa"
	local lookahead_bin_file="${EXPT_WORK_DIR}/${model_name}-lookahead.bin"
	local dict_file="${EXPT_WORK_DIR}/${model_name}.lex"
	...
	local vocab_file="${EXPT_WORK_DIR}/${model_name}.vocab"
